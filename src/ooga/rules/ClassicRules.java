 ...
 *      Must play a card that is either of same Suit or Value as the Card on top of DiscardPile.
...
public class ClassicRules implements Rule {
    ...
    public boolean isValid(Card discard, Card playedCard) {
        if(playedCard.getSuit() == discard.getSuit() || playedCard.getValue() == discard.getValue()){
            return true;
        }
        return false;
    }
